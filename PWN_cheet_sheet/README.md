...
`ROPgadget --binary <tên libc> | grep "<đoạn cần tìm>"` ( tìm ở libc )
...
- Bin (Vùng chứa chunk sau khi `free`) :
  ...
  + Large/Small bin : Chia ra từ unsorted bin ( Khi 1 chunk vào `unsorted bin` sau đó `malloc()` với size > `unsorted bin` thì sẽ cho chunk đó vào `large` nếu >= 1024 và `smallbin` nếu ngược lại 
